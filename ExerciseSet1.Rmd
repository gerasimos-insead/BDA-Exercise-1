...
**Your Answers here:**
1. This is happening in Line 23 where R creates the tmpdata matrix based on the "ticker_t_get_value". This variable gets the values of the mytickers vector in line 21. The vector mytickers has been defined in line 8 with arguments "SPY" and "APPL"
...
2.
Cumulative returns `round(100*sum(StockReturns[,"AAPL"]),1)` = 596.5 %
Average daily returns `round(100*mean(StockReturns[,"AAPL"]),3)` = 0.148 %
Standard deviation of the daily returns `round(100*sd(StockReturns[,"AAPL"]),3)` = 2.389 %

3. 
In dataSet1.R we need to change line 8 as following: mytickers = c("SPY", "AAPL", "YHOO")

Then we need to save the changes and rerun the chunk with source("dataSet1.R")

Last we need to calculate the stats

Cumulative returns `round(100*sum(StockReturns[,"YHOO"]),1)` = 291.2 %
Average daily returns `round(100*mean(StockReturns[,"YHOO"]),3)` = 0.072 %
Standard deviation of the daily returns `round(100*sd(StockReturns[,"YHOO"]),3)` = 3.019 %
...
**Your Answers here:**
1. Number of rows 'nrow(transposedData)' = 2 (This does not include the Yahoo prices)
   Number of columns 'ncol(transposedData)' = 4033

2. 'transposed_portfolio = apply(transposedData,2,mean)'
...
**Your Answers here:**
1. The startDate value in Line 9 must be set to 'startDate = "2001-01-01' 

   Then we need to save the changes and rerun the chunk with source("dataSet1.R")

2. In dataSet1.R we will change line 8 as following: mytickers = c("SPY", "AAPL", "YHOO", "TSLA", "FB")

  Then we will save the changes and rerun the chunk with source("dataSet1.R")
  Last, we calculate the returns of the equal weighted portfolio 'new_portfolio = apply(StockReturns,1,mean)' and we plot them  'pnl_plot(new_portfolio)'
...
**Your Answers here:**
1. 
First, in the read.csv function I need to change the sep=";" argument to sep="," because this is a comma separated file

Next, I need to remove the first column of myData table 'myData <- myData[,-1]' to make it the same dimension as the StockReturns[1:20,] table. 

It should return 0 because MyData and StockReturns[1:20,] should be identical, so the sum of the values that are different between the two tables should be zero. However, when I run the code the two variables are of different classes and I get a wrond result

2. 
It will create a new matrix that will have the same dimensions as Mydata, but will add the the first twenty values of the SPY retunrs to the first column (SPY) and then the next twenty values of the SPY returns to the second column of the table (AAPL)
